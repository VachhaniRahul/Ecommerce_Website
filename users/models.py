...
import uuid
from base.emails import send_profile_activation_email
...
class Cart(BaseModel):
    ...
    def get_cart_total(self):
        ...
        if self.coupon:
            if sum(price) < int(self.coupon.minimum_amount):
                self.coupon = None
                self.save()
                return sum(price), 0, sum(price)
            return sum(price) - int(self.coupon.discount_price), self.coupon.discount_price, sum(price)
        return sum(price), 0, sum(price)
